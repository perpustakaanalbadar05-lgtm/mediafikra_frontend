...
const EMPTY = { judul: '', penulis: '', kategori: '', tahun: '', deskripsi: '', cover: '' };
...
export default function AdminPortofolioPage() {
  ...
  const openEdit = (item) => {
    setForm({ judul: item.judul, penulis: item.penulis, kategori: item.kategori || '', tahun: item.tahun || '', deskripsi: item.deskripsi || '', cover: item.cover || '' });
    setEditId(item.id);
    setShowModal(true);
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editId) await api.put(`/admin/portfolios/${editId}`, payload);
      else await api.post('/admin/portfolios', payload);
      setShowModal(false);
      fetch();
    } catch (_) {}
        ...
        {loading ? Array(5).fill(0).map((_, i) => <div key={i} className="bg-white rounded-2xl border border-slate-100 h-48 animate-pulse" />) :
          items.map(item => (
            <div key={item.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden group">
              <div className="h-36 bg-gradient-to-br from-indigo-100 to-violet-100 flex items-center justify-center relative">
